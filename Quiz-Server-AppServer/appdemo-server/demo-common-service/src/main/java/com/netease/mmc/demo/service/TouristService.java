...
public class TouristService {
     ...
     * 根据账号，查询玩家昵称
     *
     * @param accids 需要查询的账号列表
     * @return
     */
    public Map<String, String> queryNickname(List<String> accids) {
        if (CollectionUtils.isEmpty(accids)) {
            return Collections.emptyMap();
        }
        Map<String, String> nicknameMap = Maps.newHashMapWithExpectedSize(accids.size());

        List<TouristDO> touristDOS = touristDao.listByAccids(accids);
        for (TouristDO touristDO : touristDOS) {
            nicknameMap.put(touristDO.getAccid(), touristDO.getNickname());
        }
        return nicknameMap;
    }
    ...
    private String produceAccid() {
        // 递增序列号 + 2位随机数
        return CommonConst.TOURIST_USER_NAME_PREFIX + seqService.getSeqId() + (new Random().nextInt(90) + 10);
    }
