...
public class RedissonUtil {
     ...
     * @return
     */
    public static <V> boolean sadd(String key, V value) {
        Preconditions.checkArgument(StringUtils.isNotEmpty(key), KEY_CAN_NOT_BE_EMPTY);
        Redisson redisson = getRedisson();
        RSet<V> set = redisson.getSet(getKeyWithNamespace(key));
    ...
    public static void sdel(String key) {
        Preconditions.checkArgument(StringUtils.isNotEmpty(key), KEY_CAN_NOT_BE_EMPTY);
        Redisson redisson = getRedisson();
        RSet<Object> set = redisson.getSet(getKeyWithNamespace(key));
        set.delete();
    }

    /**
     * 设置集合过期时间，注意只有集合存在时设置才会生效
     *
     * 空集合会被自动删除
     *
     * @param key
     ...
     * @return
     */
    public static boolean sexpire(String key, long seconds) {
        Preconditions.checkArgument(StringUtils.isNotEmpty(key), KEY_CAN_NOT_BE_EMPTY);
        Redisson redisson = getRedisson();
        RSet<Object> set = redisson.getSet(getKeyWithNamespace(key));
        return set.expire(seconds, TimeUnit.SECONDS);
    }
     ...
     * @return -1不失效，-2key不存在
     */
    public static long sttl(String key) {
        Preconditions.checkArgument(StringUtils.isNotEmpty(key), KEY_CAN_NOT_BE_EMPTY);
        Redisson redisson = getRedisson();
        RSet<Object> set = redisson.getSet(getKeyWithNamespace(key));
        return set.remainTimeToLive();
    }
    ...
    public static <K, V> RMap<K, V> getMap(String key) {
        Preconditions.checkArgument(StringUtils.isNotEmpty(key), KEY_CAN_NOT_BE_EMPTY);
        Redisson redisson = getRedisson();
        return redisson.getMap(getKeyWithNamespace(key));
    }
}
