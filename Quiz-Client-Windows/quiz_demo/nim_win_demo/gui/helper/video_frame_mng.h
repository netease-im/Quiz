...
public:
	...
	* @return void	无返回值
	*/
	void AddVideoFrame(const char* data, int size, int width, int height, FrameType frame_type = Ft_ARGB_r);
	void AddVideoFrameEx(const char* data, int size, int width, int height, int32_t ls_type);
	...
	* @return void	无返回值
	*/
	bool GetVideoFrame(uint64_t& time, char* out_data, int& width, int& height, bool mirror = false, bool argb_or_yuv = true);
private:
	nbase::NLock  lock_;
