...
typedef enum enum_NLSS_VIDEOIN_TYPE{
	...
	EN_NLSS_VIDEOIN_DECKLINK,                  //!< DeckLink视频采集卡设备
...
typedef enum enum_NLSS_AUDIOIN_TYPE{
	...
	EN_NLSS_AUDIOIN_DECKLINK                   //!< DeckLink设备的音频
...
*/
typedef struct struct_NLSS_DECKLINK_PARAM
{
	char        *paDeviceName;    //!< 设备名.
	char        *paDeviceId;      //!< 设备Id
	int         iDeviceMode;      //!< 设备打开模式
...
typedef struct struct_NLSS_AUDIOIN_PARAM
{
    ...
    char                   *paaudioDeviceName;    //!< 当音频源为麦克风设备时，需设置麦克风设备名称；当音频源为DeckLink采集卡设备时，需设置DeckLink设备Id
...
typedef struct struct_NLSS_PARAM
{
    ...
    bool                      bSyncTimestamp;     //!< 同步时间戳透传开关，默认关闭，推流类型必须包含视频，同时需要网易云播放器支持
} ST_NLSS_PARAM;
...
* 设备模式
*/
typedef struct struct_NLSS_INDEVICE_MODE_INF
{
	const char    *paModeName;
	int           iMode;
