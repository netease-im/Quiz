...
*
*  @param  pstVideoDevices: 视频设备信息 出参
*  @param  iMaxVideoDevicesNum: pstVideoDevices最大容量 入参
*  @param  pstAudioDevices: 音频设备信息 出参
...
/**
*  获取所有DeckLink设备信息以及每个设备的模式
*
*  @param  pstDeckLinkDevices: DeckLink设备信息；当此参数为空时，iMaxDeckLinkDeviceNum返回可用的DeckLink设备个数 出参
*  @param  iMaxDeckLinkDeviceNum: 作为入参是pstDeckLinkDevices的最大容量；作为出参是最终取到的设备信息个数
*
*  @return NLSS_RET NLSS_OK成功，NLSS_ERR失败
*/
...
*
*  @param  pstDeckLinkDeviceModes: DeckLink设备的模式 出参
*  &param  paDeviceId: 指定的DeckLink设备id 入参
*  @param  iMaxDeckLinkDeviceModeNum: 作为入参是pstDeckLinkDeviceModes的最大容量；作为出参是最终取到的模式个数
*
*  @return NLSS_RET NLSS_OK成功，NLSS_ERR失败
*/
EXPORTS_API   NLSS_RET  Nlss_GetDeckLinkDeviceModeListById(NLSS_OUT ST_NLSS_INDEVICE_MODE_INF *pstDeckLinkDeviceModes, const char* paDeviceId, NLSS_IN_OUT int* iMaxDeckLinkDeviceModeNum);
...
*
*  @param  hNLSService: 直播推流实例，入参
*  @param  iRatio: 暂定0-100；0表示静音，100表示原始音量
*
*  @return 无
*/
...
*  @param  hNLSService: 直播推流实例，入参
*
*  @return 当前直播流的pts，单位毫秒
*/
EXPORTS_API  unsigned long long   Nlss_GetStreamPts(_HNLSSERVICE hNLSService);

/**
*  获取当前透传的同步时间戳（通过视频流透传，ST_NLSS_PARAM::bSyncTimestamp设为true时，此接口有效），单位毫秒
*
*  @param  hNLSService: 直播推流实例，入参
*
*  @return 当前透传的同步时间戳，单位毫秒
*/
